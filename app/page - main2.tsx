...
export default function TerminalPage() {
  ...
  // ✨ UPDATED useEffect: Handles smooth scrolling and shake reset
  useEffect(() => {
    ...
    // Reset shake state after the animation runs (0.2s is the duration in globals.css)
    if (isShaking) {
        const timer = setTimeout(() => setIsShaking(false), 200);
        return () => clearTimeout(timer);
    }
  }, [history, currentModalVideo, isShaking]);
  ...
  // ✨ NEW: Handler to select all text on focus for easy copy-pasting
  ...
  const executeCommand = useCallback((cmd: string) => {
    ...
    switch (baseCommand) {
      ...
      default:
        // ✨ NEW: Trigger shake effect on unrecognized command
output = (
          <span className="text-red-400">
            Error: Command not recognized: {baseCommand}. Type 'list' for help.
          </span>
        );
    }
  ...
  return (
    <>
      ...
      <div 
        // Apply the CRT effect class here (if it's not applied to the <body> in layout.tsx)
        className="flex flex-col h-screen bg-black text-green-400 font-mono p-4 overflow-hidden terminal-crt" 
        onClick={() => inputRef.current?.focus()}
      >
        <div 
          ref={terminalRef} 
          // ✨ MODIFIED: Conditional class for shake and overflow fix (hides scrollbar when shaking)
          className={`flex-grow whitespace-pre-wrap leading-relaxed pb-4 ${isShaking ? 'shake overflow-hidden' : 'overflow-y-auto'}`}
        >
          ...
          <div className="flex items-center">
            ...
            <div className="relative flex-grow">
                ...
                <input
                    ...
                    onFocus={handleFocus} // ✨ NEW: Selects text on click for easy copy-pasting
                    
                    className="bg-transparent border-none outline-none flex-grow w-full text-transparent relative z-20 caret-green-400 terminal-input"
                    autoFocus
                    spellCheck="false"
                    autoComplete="off"
                />
